...
vector<mzd_t *> makeAds(const int len, const Gf2Matrix &_Ms) {
  vector<mzd_t *> Ads;
  ...
  for (int i = 0; i < len; i++) {
    ...
    mzd_t *Ad = mzd_mul(NULL, di, Msi, cutoff);
    Ads.push_back(Ad);
    mask <<= 1;
...
int main() {
  ...
  const int n = 17;
  const int blockcount = 1 << n; // blockcount = 2^17
  const int taglen = 4;
  ...
  vector<mzd_t *> Ads = makeAds(n*128, Ms);
  ...
  unsigned char *forged_ciphertext = new unsigned char[ciphertext_len];
  unsigned char *forged_res = new unsigned char[ciphertext_len];
  ...
  for (int c = 0; 1; c++) {
	...
	for (int j = 0; j < Tbasis.height(); j++) {
      ...
      memcpy(forged_ciphertext, ciphertext, ciphertext_len);
	...
	*(forged_ciphertext+(blockcount-1-block)*16+byte) ^= mask;
	byte = ++byte % 16;
	mask = 0;
      }
      ...
      // to complete challenge in reasonable time
      int forgery_decrypt_len = decrypt(forged_ciphertext, ciphertext_len, tag, key, nonce, forged_res);
